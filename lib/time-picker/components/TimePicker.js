...
const TimePicker = ({ time, onChange }) => {
  ...
  React.useEffect(() => {
    ...
    const [hours, minutes] = time.slice(0, -2).trim().split(":");
    const formattedTime = `${zeroPadTime(hours)}:${minutes}`;
...
TimePicker.propTypes = {
  time: (props, propName, componentName) => {
    ...
    if (!isTimeFormat(time.slice(0, -2))) {
      ...
      return new Error(
        `Invalid Time Format supplied to ${componentName}.  Please use hh:mm AM|PM  3:30 AM`
      );
    }
  },
