...
const Slider = ({ startStep, endStep, onChangeValue, initialValue, label }) => {
  ...
  React.useEffect(() => {
    initialValue = initialValue > endStep ? endStep : initialValue;
    initialValue = initialValue < startStep ? startStep : initialValue;
    onChangeValue(initialValue);
    setOffset(Math.floor(stepDistance / 10));
  ...
  const onSliderMove = (position) => {
    ...
    if (sliderPosition !== trackPosition.lastValue) {
      setThumbDistance(sliderPosition - TRACK_OFFSET);
    ...
    if (stepValue >= startStep && stepValue <= endStep)
      onChangeValue(stepValue);
  };

  const onMouseMove = (event) => {
    const position = event.pageX;
    if (isInRange(position, offset)) {
  ...
  const isInRange = (position, offset) => {
    if (
      position >= trackPosition.left - offset &&
      position <= trackPosition.left + TRACK_WIDTH + offset * 2 &&
      clientStart
    ) {
      return true;
    }
