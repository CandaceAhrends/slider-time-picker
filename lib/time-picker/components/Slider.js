...
const Slider = ({ startStep, endStep, onChangeValue, initialValue, label }) => {
  ...
  const [stepDistance, setStepDistance] = React.useState(0);

  React.useEffect(() => {
    onChangeValue(initialValue);
    let steps = endStep - startStep + 1;
    setStepDistance(steps);

    if (initialValue < endStep) {
      const initialPercentage = whatPercentOfNumber(initialValue, steps);
      console.log("set value", initialPercentage, initialValue, steps);
  ...
  const onSliderMove = (position) => {
    const length = trackPosition.right - trackPosition.left;
    const sliderPosition = position - trackPosition.left;
    setThumbDistance(sliderPosition);
    const trackPercentMoved = whatPercentOfNumber(sliderPosition, length);
